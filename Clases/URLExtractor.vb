...
Public Class URLExtractor
    ...
    Private Shared ENC_XOR As Byte() = New Byte() {CByte(12), CByte(57), CByte(251), CByte(120), _
                                                   CByte(18), CByte(75), CByte(6), CByte(250), _
                                                   CByte(85)} ' REMOVED FROM SOURCE CODE, SORRY
    ...
    Private Shared ReadOnly patternHTTPURI() As String =
        {"(http://|https://|)mega.co.nz/(?<MODE2>#|#F|#N)!(?<FileID>[^\!]+)(!(?<FileKey>[\w-#=]+))?",
         "(http://|https://|)mega.nz/(?<MODE2>#|#F|#N)!(?<FileID>[^\!]+)(!(?<FileKey>[\w-#=]+))?",
         "(http://|https://|)mega.nz/(?<MODE2>file|folder)/(?<FileID>[^\#]+)(#(?<FileKey>[\w-#=]+))?",
         "(http://|https://|)megashur.se/out.php\?m=(?<FileID>[^\!]+)(!(?<FileKey>[\w-#=]+))?",
         "chrome://mega/content/secure.html(?<MODE2>#|#F|#N)!(?<FileID>[^\!]+)(!(?<FileKey>[\w-#=]+))?"}

    Private Shared ReadOnly patternMEGAURI() As String =
        {"(?<TAG>mega)(?<MODE1>://|:///|:)(?<MODE2>#|#F|F|#N|N)!(?<FileID>[^\!]+)(!(?<FileKey>[\w-#=]+))?",
        "(?<TAG>mega)(?<MODE1>://|:///|:)(?<MODE2>file|folder|#file|#folder)/(?<FileID>[^\#]+)(#(?<FileKey>[\w-#=]+))?",
        "(?<TAG>mega)(?<MODE1>://|:///|:)(?<MEGASEARCH>mega-search(\?|/\?))(?<MEGASEARCH_FILEID>[\w-#=]+)",
        "(?<TAG>mega)(?<MODE1>://|:///|:)(?<BASIC_ENCODE>enc(\?|/\?))(?<ENCODED_FILEID>[\w-#=]+)",
        "(?<TAG>mega)(?<MODE1>://|:///|:)(?<BASIC_ENCODE>enc2(\?|/\?))(?<ENCODED_FILEID>[\w-#=]+)",
        "(?<TAG>mega)(?<MODE1>://|:///|:)(?<BASIC_ENCODE>fenc(\?|/\?))(?<ENCODED_FILEID>[\w-#=]+)",
    ...
    Public Shared Function EsUrlAcortador(ByVal URL As String) As Boolean
        ...
        If URL.ToLower.Contains("bit.ly") Then Return True ' De momento solo soportamos bitly
        If URL.ToLower.Contains("goo.gl") Then Return True
        Return False
    End Function
